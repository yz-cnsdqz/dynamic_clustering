...
struct Global {
    ...
    int MIN_KEYFRAME_POOLSIZE = 10;   //  Achieving this number triggers clustering algorithm and then releases this buffer. 
    ...
    std::vector<int> timeStampsInBuffer;
    std::vector<int> timeStampsInPool;
    std::vector<int> newClusterIndex;
    std::vector<int> newMovementIndex;
    ...
    struct Codebook {
        ...
        std::vector<int> cluster_frame_timestamp;
        std::vector<cv::Mat> cluster_frame_feature;
...
class SortingIndexExtractor2{
std::vector<std::pair<A, B>> _vec;
public:
    SortingIndexExtractor2(std::vector<std::pair<A, B>>& vec_in) : _vec(vec_in){}
...
void recordClusterCloestFrameIndex( ) {
    ...
    float _distn; 
    int idx = 0;
    int n_samples = global.output_vec_KFramePool.size();
    ...
    for (int i = 0; i < global.codebook.n_clusters; i++){
        ...
        for (int j = 0; j < n_samples; j++) {
            ...
            if (global.codebook.init_sample_labels.at<float>(j)==(float)i) {
                cv::Mat jtst;
                jtst = global.output_vec_KFramePool[j];
                // jts = global.output_vec_KFramePoolS
        ...
        global.codebook.cluster_frame_timestamp.push_back(global.timeStampsInPool[idx]);
        global.codebook.cluster_frame_feature.push_back(global.output_vec_KFramePool[idx]);
    ...
    // for (int k = 0; k < global.cointk.cluster_frame_index.size(); k++) {
    // //     std::cout << "cluster - " << k+1 << std::endl;
    //     cv::imshow("codebook image", global.cointk.cluster_frame_index[k].img);
    //     cv::waitKey(0);
    // }
}
...
//     // std::cout << "[CODEBOOK INFO] cointk.cluster_frame_index.size() = " << global.cointk.cluster_frame_index.size() << std::endl;
//     // FrameCompare comp;
//     // std::priority_queue<Frame, std::vector<Frame>, FrameCompare> buffer(comp);
//     for (int j = 0; j < global.cointk.cluster_frame_index.size(); j++) 
//         buffer.push(global.cointk.cluster_frame_index[j]);
...
//         cv::imshow("Codebook Image", global.cointk.cluster_frame_index[k].img);
//         std::cout << "video_frame_index = " << global.cointk.cluster_frame_index[k].video_frame_number << std::endl;
//         // if(global.SAVE_IMAGE) {
//         //     out_name << "output/codebook_cluster_" << k << ".png";
//         //     std::cout << out_name.str()<<std::endl;
//             cv::imwrite(out_name.str(), global.cointk.cluster_frame_index[k].img);
//         //     out_name.str("");
//         // }
...
void getKeyFrameIdxFromBuffer(const cv::Mat& memory, const cv::Mat& movement, std::vector<int>& frame_idx){
    ...
    for(int i = 0; i < memory.rows; i++){
        ...
        _curve.push_back(float(memory.at<float>(i)));
    ...
    for(int i = 0; i < global.output_vec_KFrameBuffer.size(); i+= global.KEYFRAME_SAMPLING_RATE){
...
// Work: (1) temporal interpolation and (2) keyframe selection
{
    ...
    int n_frames = global.output_vec_KFrameBuffer.size();
    ...
    if (global.VERBOSE)
        std::cout << "[DEBUG] jointsTemporalSmoothingKeyframeSelection: begin to begin to run" << std::endl;
    for (int i = 0; i < n_frames; i++)
    ...
    if (global.VERBOSE){
        cv::Mat frame_idx_mat(frame_idx);
        std::cout<< "KEYFRAME_POOL_FILLTIMES="<< global.KEYFRAME_POOL_FILLTIMES  << std::endl;
        std::cout<< "selected_frame_idx=" << std::endl;
        std::cout << frame_idx_mat<<std::endl;
    }
    ...
    for (int i = 0; i < frame_idx.size(); i++) {
        global.output_vec_KFramePool.push_back(global.output_vec_KFrameBuffer[frame_idx[i]]);
        global.timeStampsInPool.push_back(global.timeStampsInBuffer[frame_idx[i]]);
    ...
    // sort the selected frames according to the time stamp
    // std::vector<std::pair<int, cv::Mat>> frames;
    // for (int i = 0; i < global.timeStampsInPool.size(); i++){
    //     frames.push_back(std::make_pair(global.timeStampsInPool[i], global.output_vec_KFramePool[i]));
    // }

    // SortingIndexExtractor2<int, cv::Mat> idx_sorter(frames);
    // std::sort(frames.begin(), frames.end(),idx_sorter);
    
    // for (int i = 0; i < global.timeStampsInPool.size(); i++){
    //     frames.push_back(std::make_pair(global.timeStampsInPool[i], global.output_vec_KFramePool[i]));
    //     global.timeStampsInPool[i] = frames[i].first;
    //     global.output_vec_KFramePool[i] = frames[i].second;
    // }
    ...
    global.timeStampsInBuffer.clear();
...
void initCodebook ( ) {
    ...
    cv::Mat samples = cv::Mat::zeros(global.output_vec_KFramePool.size(), global.num_dims, CV_32F); // the detection score is included

    for (int i = 0; i < global.output_vec_KFramePool.size(); i++) 
       global.output_vec_KFramePool[i].copyTo(samples.row(i));
    ...
    for(int i = 0; i < global.output_vec_KFramePool.size(); i++) {
        ...
        if ( _dist_min > _radius_max) { // create a new cluster with new location, std=0 and image
            ...
            if (global.VERBOSE){
                ...
                // std::cout << "global.timeStampsInPool[i]=" << global.timeStampsInPool[i] << std::endl;
            }

            
            global.newClusterIndex.push_back(global.timeStampsInPool[i]);
            global.newMovementIndex.push_back(global.timeStampsInPool[i]);
            ...
            global.codebook.cluster_frame_timestamp.push_back(global.timeStampsInPool[i]);
            global.codebook.cluster_frame_feature.push_back(_joints_t);
            ...
            // new transition is detected
            // if (global.codebook.transition_map.ref<float>(transition_map_idx)==0.0f)
            //     global.newMovementIndex.push_back(global.timeStampsInPool[i]);

            // if transition between cluster occurs
            if (transition_map_idx[0] != transition_map_idx[1])
                global.newMovementIndex.push_back(global.timeStampsInPool[i]);
            ...
            cv::Mat _cluster_img_joint = global.codebook.cluster_frame_feature[_dist_min_idx];
            
            // _cluster_img_joint = global.
            // std::cout<< "[DEBUG] updateCodebook(): _cluster_img_joint=" << _cluster_img_joint<< std::endl;
            float _dist_img2center = cv::norm(_cluster_img_joint, _mean);
            if (_dist_img2center > (_dist_min) ){
                global.codebook.cluster_frame_timestamp[_dist_min_idx] = global.timeStampsInPool[_dist_min_idx];
                global.codebook.cluster_frame_feature[_dist_min_idx] = _joints_t;
            }
...
void mexFunction(int nlhs, mxArray* plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    if (nrhs != 5)
        mexErrMsgIdAndTxt("Three arguments should be parsed", "usage: fun(samples, BufferSize, thresh_from_buffer (0-1), uncertainty, VERBOSE)");
    ...
    global.thresh_from_buffer = (float)*mxGetPr(prhs[2]);
    int offset = global.KEYFRAME_BUFFERSIZE;
    float error_estimate = (float)*mxGetPr(prhs[3]);
    ...
    global.timeStampsInBuffer.clear();
    global.timeStampsInPool.clear();
    global.newClusterIndex.clear();
    global.newMovementIndex.clear();
    global.codebook.cluster_frame_timestamp.clear();
    global.codebook.cluster_frame_feature.clear();
    // step2: run the incremental learning mechanism
    // std::cout << "-------- run dynamical clustering-----------" << std::endl;
    for(int i = 0; i < global.num_samples; i+=offset){
        ...
        for(int j = 0; j < samples_for_buffer.rows; j++){
            ...
            global.timeStampsInBuffer.push_back(i+j);
        }
    ...
    if (global.VERBOSE)
        std::cout << "after running clustering" << std::endl;
    // cv::Mat labels;
    cv::Mat cluster_frame_timestamp (global.codebook.cluster_frame_timestamp);
    cv::Mat cluster_locs;
    cv::Mat newClusterIndex(global.newClusterIndex);
    cv::Mat newMovementIndex(global.newMovementIndex);
    // global.codebook.sample_labels.convertTo(labels, CV_64F);
    global.codebook.cluster_locs.convertTo(cluster_locs, CV_64F);
    newClusterIndex.convertTo(newClusterIndex, CV_64F);
    newMovementIndex.convertTo(newMovementIndex, CV_64F);
    ...
    plhs[0] = ocvMxArrayFromMat_int32(cluster_frame_timestamp);
    plhs[1] = ocvMxArrayFromMat_double(cluster_locs);
    plhs[2] = ocvMxArrayFromMat_double(newClusterIndex);
    plhs[3] = ocvMxArrayFromMat_double(newMovementIndex);
    // std::cout << "-------- run dynamical clustering--------end" << std::endl;
}
